...
function ConstructTerminalInstance(nBot) {
	...
	term.promptInputPrefix = nBot.settings.terminalInputPrefix||'> ';
	term.promptBufferArr = [""];
	term.promptBufferCurrent = 0;
	term.promptBufferMax = 10;
	term.cursorPositionAbsolute = 1;
	term.promptBufferCurrentUnModifiedState = "";
	...
	term.updatePrompt = function (){
		var promptPrefix = term.promptInputPrefix;
		...
		var promptCursorPos = term.getPromptCursorPos();
		var promptBuffer = term.promptBufferArr[term.promptBufferCurrent];
		var promptData = promptBuffer.substr(promptCursorPos[1], availColumns);
		process.stdout.write("\x1b[1G\x1b[2K");
		process.stdout.write(promptPrefix);
		process.stdout.write(promptData);
		process.stdout.write("\x1b["+promptCursorPos[0]+"G");
	};
	
	term.getPromptCursorPos = function (){
		var promptPrefixLen = +term.promptInputPrefix.length;
		var termColumns = +process.stdout.columns;
		var availColumns = termColumns-promptPrefixLen;
	...
	term.processLine = function (data) {
	...
	term.processInput = function (data) {
		if (data == "\x0d") {
			//enter
			if (term.promptBufferArr[term.promptBufferCurrent]) {
				process.stdout.write('\x0a');
				var BufferData = term.promptBufferArr[term.promptBufferCurrent];
				if (term.promptBufferArr[term.promptBufferCurrent] !== "") {
					term.promptBufferArr.splice(1, 0, term.promptBufferArr[term.promptBufferCurrent]);
					if (term.promptBufferCurrent > 0) {
						term.promptBufferArr[term.promptBufferCurrent+1]=term.promptBufferCurrentUnModifiedState;
					}
					term.promptBufferArr.splice((term.promptBufferMax+1), 1);
				}
				term.promptBufferCurrent=0;
				term.promptBufferArr[0]="";
				term.cursorPositionAbsolute=1;
				term.updatePrompt();
				term.processLine(BufferData);
			}
		}else if (data == "\x7f") {
			//backspace
			term.promptBufferArr[term.promptBufferCurrent]=
			term.promptBufferArr[term.promptBufferCurrent].substr(0, (term.cursorPositionAbsolute-2))+
			term.promptBufferArr[term.promptBufferCurrent].substr((term.cursorPositionAbsolute-1));
			...
			//del
			term.promptBufferArr[term.promptBufferCurrent]=
			term.promptBufferArr[term.promptBufferCurrent].substr(0, (term.cursorPositionAbsolute-1))+
			term.promptBufferArr[term.promptBufferCurrent].substr((term.cursorPositionAbsolute));
			term.updatePrompt();
		}else if (data == "\x1b\x5b\x41") {
			//up arrow
			if (term.promptBufferCurrent < term.promptBufferMax && term.promptBufferArr[term.promptBufferCurrent+1] !== undefined) {
				term.promptBufferCurrent++;
				term.promptBufferCurrentUnModifiedState = term.promptBufferArr[term.promptBufferCurrent];
				term.cursorPositionAbsolute=term.promptBufferArr[term.promptBufferCurrent].length+1;
				term.updatePrompt();
			}
			...
			//right arrow
			if (term.promptBufferArr[term.promptBufferCurrent].length >= term.cursorPositionAbsolute) {
				term.cursorPositionAbsolute++;
			}
			term.updatePrompt();
	...
	term.readInput = function () {
		var data = process.stdin.read();
		if (data !== null) {
			term.processInput(data);
		}
	};
	...
	(function () {
		...
		process.stdin.on('readable', term.readInput);
	})();
}
