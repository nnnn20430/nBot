...
function CreateBotInstance(options) {
	...
	bot.ircReceiveNumHandle001_001 = function (data) {
	...
	bot.ircReceiveNumHandle005_005 = function (data) {//RPL_ISUPPORT
	...
	bot.ircReceiveNumHandle311_318 = function (data) {//RPL_WHOISUSER
	...
	bot.ircReceiveNumHandle352_315 = function (data) {//RPL_WHOREPLY
		bot.emitBotEvent('botReceivedNum352', data);
		bot.ircResponseListenerEmit('352', data);
	};

	bot.ircReceiveNumHandle353_366 = function (data) {//RPL_NAMREPLY
		bot.emitBotEvent('botReceivedNum353', data);
		bot.ircResponseListenerEmit('353', data);
	};

	bot.ircReceiveNumHandle364_365 = function (data) {//RPL_LINKS
	...
	bot.ircDataReceiveHandle = function (data) {
		...
		function ircNumericHandle(data) {
			...
			if (iMLMBuffer) {
				if (iNMH[iMLMBuffer[0]] == data[2]) {
					var handle = iMLMBuffer[0]+'_'+iNMH[iMLMBuffer[0]];
					bot['ircReceiveNumHandle'+handle](iMLMBuffer);
					iMLMBuffer = null;
