...
function ConstructShellInstance(im) {
	...
	shell.currentConnection = 0;
	shell.lastChannel = im.iOpts[shell.currentConnection].channels[0];
	...
	shell.processLine = function (data) {
		...
		var cConnsettings = im.iOpts[shell.currentConnection];
		var connectionName = cConnsettings.connectionName||shell.currentConnection;
		var bot;
		if (im.iArr[shell.currentConnection]) {
			bot = im.iArr[shell.currentConnection];
		}
				...
				case 'say':
					(function () {
						if (commandArgs[2] !== undefined) {
							if (bot && !bot.ircConnection.destroyed) {
								shell.log('['+connectionName+':'+commandArgs[1]+'] '+cConnsettings.botName+': '+commandArgs[2]);
				...
				case 'connection':
					(function () {
						...
						if (commandArgs[1] !== undefined) {
							...
							bot = im.iArr[shell.currentConnection];
							shell.log('> Current connection id: '+shell.currentConnection+', name: "'+connectionName+'", status: '+(bot?(bot.ircConnection.destroyed?'dead':'alive'):'dead')+'.');
						}
					})();
					break;
				case 'fakemsg':
					...
					(function () {
						...
						if (bot) {
							if (bot.plugins[plugin]) {
								...
								bot.pluginLoad(plugin, cConnsettings.pluginDir+'/'+plugin+'.js');
							}
						} else {
							shell.log('Current connection is dead.');
						}
		...
		if (data && data.charAt(0) != '/') {
			if (bot && !bot.ircConnection.destroyed) {
				shell.log('['+connectionName+':'+shell.lastChannel+'] '+im.iOpts[shell.currentConnection].botName+': '+data);
